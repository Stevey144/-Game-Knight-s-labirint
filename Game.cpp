...
public:
    ...
    Character(Map& map) : x_pos(1), y_pos(1), h(74), w(75), speedMove(0.05), speedRun(1), dir(0), dx(1), dy(0), Picture_name("mages.png"), MAP(map) {
        playerScore = 0;
        HasKey = 0;

        if (!characterImage.loadFromFile("pictures/" + this->Picture_name)) {
            std::cerr << "Error loading image";
        }
        ...
        for (int i = y / PICTURE_RESOLUTION; i < (y + h) / PICTURE_RESOLUTION; i++)
            for (int j = x / PICTURE_RESOLUTION; j < (x + w) / PICTURE_RESOLUTION; j++) {
                ...
                if (MAP.Grid[i][j] == 't' && HasKey) {
                    // Add treasure interaction logic here
                }
            }
    }
